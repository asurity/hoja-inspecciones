...
'' ----------------------------------------------------------------------
' Subrutina: ProtectSheet
' Propósito: Protege una hoja de Excel aplicando restricciones estrictas y usando la contraseña proporcionada.
' Argumentos:
'   - ws: Referencia a la hoja a proteger.
'   - sheetPassword: Contraseña para proteger la hoja.
' Lógica:
'   1. Protege la hoja con opciones que restringen la mayoría de las acciones del usuario.
'   2. Permite solo el filtrado de datos (AllowFiltering:=True).
'   3. Restringe la selección de celdas para evitar modificaciones accidentales.
'   4. Si ocurre un error, lo registra en el log de errores.
'' ----------------------------------------------------------------------
'' ----------------------------------------------------------------------
...
Public Sub ProtectSheetForReading(ByRef ws As Worksheet, ByVal sheetPassword As String)
    ...
    If Not Configuration2.ENABLE_SHEET_PROTECTION Then
        Debug.Print "[PROTECT] Protección deshabilitada (MODO DESARROLLO): " & ws.Name
        Exit Sub
    End If
